    ...
    def map
        if params[:address]
            ...
            @display.xpath('//response').each do |thing|
                @get_zpid        = thing.at_xpath('//results//result//zpid').content unless thing.at_xpath('//results//result//zpid').nil?
            end
            ...
            flash[:welcome] = "Enter your property address to discover the benefits of listing your unit with the Seattle Housing Authority."
            @longitude = 42
            @latitude = -122
        end
    end
